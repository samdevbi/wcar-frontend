...
const AdminUsers: NextPage = ({ initialInquiry, ...props }: any) => {
	...
	const [value, setValue] = useState(
		membersInquiry?.search?.status ? membersInquiry?.search?.status : 'ALL',
	);
	...
	const tabChangeHandler = async (event: any, newValue: string) => {
		...
		switch (newValue) {
			case 'ACTIVE':
				setMembersInquiry({ ...membersInquiry, search: { status: Status.ACTIVE } });
				break;
			case 'BLOCK':
				setMembersInquiry({ ...membersInquiry, search: { status: Status.BLOCK } });
				break;
			case 'DELETE':
				setMembersInquiry({ ...membersInquiry, search: { status: Status.DELETE } });
				break;
			default:
				delete membersInquiry?.search?.status;
				setMembersInquiry({ ...membersInquiry });
				break;
		}
	};
		...
		try {
			...
			if (newValue !== 'ALL') {
				setMembersInquiry({
					...
					search: {
						...membersInquiry.search,
						type: newValue as Type,
					},
				});
			} else {
				delete membersInquiry?.search?.type;
				setMembersInquiry({ ...membersInquiry });
			}
		} catch (err: any) {
						...
						<Box component={'div'}>
							...
							<Stack className={'search-area'} sx={{ m: '24px' }}>
								...
								<Select sx={{ width: '160px', ml: '20px' }} value={searchType}>
									...
									<MenuItem value={'SERVICE'} onClick={() => searchTypeHandler('SERVICE')}>
										Service
									</MenuItem>
