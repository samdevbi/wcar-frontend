...
import { Status, Type } from '../../../enums/member.enum';
						...
						{members.length !== 0 &&
							members.map((member: Member, index: number) => {
								const member_image = member.image
									? `${REACT_APP_API_URL}/${member.image}`
								...
								return (
									<TableRow hover key={member?._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
										...
										<TableCell align="center">
											...
											>
												{Object.values(Type)
													.filter((ele) => ele !== member?.type)
													.map((type: string) => (
														<MenuItem
															onClick={() => updateMemberHandler({ _id: member._id, type: type })}
															key={type}
														>
										...
										<TableCell align="center">{member.warnings}</TableCell>
										<TableCell align="center">{member.blocks}</TableCell>
										<TableCell align="center">
											...
											>
												{Object.values(Status)
													.filter((ele: string) => ele !== member?.status)
													.map((status: string) => (
														<MenuItem
															onClick={() => updateMemberHandler({ _id: member._id, status: status })}
															key={status}
														>
															<Typography variant={'subtitle1'} component={'span'}>
																{status}
